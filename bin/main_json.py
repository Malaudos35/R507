...
def put_ordinateur(ordinateur: Ordinateur):
    for key,ordi in dict(ordinateurs).items():
        if ordi.ip == ordinateur.ip:
            ordinateurs[key]=ordinateur
            return {"message": "Ordinateur added successfully"}
...
def delete_ordinateur(ip: str):
    global ordinateurs
    ordinateurs = [ordinateur for ordinateur in ordinateurs if ordinateur.ip != ip]
